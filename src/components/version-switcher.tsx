...
import { LayoutBottomIcon, Tick02Icon, UnfoldMoreIcon } from "@hugeicons/core-free-icons";
          ...
          <DropdownMenuContent align="start">
            {versions.map((version) => (
              <DropdownMenuItem key={version} onSelect={() => setSelectedVersion(version)}>
                v{version}{" "}
                {version === selectedVersion && <HugeiconsIcon icon={Tick02Icon} strokeWidth={2} className="ml-auto" />}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
